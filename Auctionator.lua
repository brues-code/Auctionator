...
local function Atr_SlashCmdFunction(msg)
	...
	local param1 = param1u and string.lower(param1u) or nil;
	local param2 = param2u and string.lower(param2u) or nil;
	local param3 = param3u and string.lower(param3u) or nil;
...
function Atr_InitScanDB()
	...
	-- UnitFactionGroup("player") can return nil at ADDON_LOADED time before the
	-- player-data update has arrived. GetRealmName() should always be set, but
	-- guard it too for safety.
	local realm_Faction = (GetRealmName() or "UnknownRealm").."_"..(UnitFactionGroup("player") or "Neutral");
...
function Atr_OnLoad()
	...
	-- In 1.12 SavedVariablesPerCharacter restore can leave globals nil for
	-- brand-new characters; ensure the migration check below has a number
	-- to compare against.
	AUCTIONATOR_SHOW_TIPS	= AUCTIONATOR_SHOW_TIPS		or 1;
	AUCTIONATOR_OPEN_FIRST	= AUCTIONATOR_OPEN_FIRST	or 0;
	AUCTIONATOR_OPEN_BUY	= AUCTIONATOR_OPEN_BUY		or 0;
	AUCTIONATOR_DEF_DURATION = AUCTIONATOR_DEF_DURATION or "N";
	AUCTIONATOR_DEFTAB		= AUCTIONATOR_DEFTAB		or 1;
	AUCTIONATOR_ENABLE_ALT	= AUCTIONATOR_ENABLE_ALT	or 1;
	AUCTIONATOR_OPEN_ALL_BAGS = AUCTIONATOR_OPEN_ALL_BAGS or 1;
	AUCTIONATOR_SHOW_ST_PRICE = AUCTIONATOR_SHOW_ST_PRICE or 0;
	AUCTIONATOR_V_TIPS		= AUCTIONATOR_V_TIPS		or 1;
	AUCTIONATOR_A_TIPS		= AUCTIONATOR_A_TIPS		or 1;
	AUCTIONATOR_D_TIPS		= AUCTIONATOR_D_TIPS		or 1;
	AUCTIONATOR_SHIFT_TIPS	= AUCTIONATOR_SHIFT_TIPS	or 1;
	AUCTIONATOR_DE_DETAILS_TIPS = AUCTIONATOR_DE_DETAILS_TIPS or 4;
...
function Atr_AuctionFrameTab_OnClick (arg1, arg2)

	-- We install this as the global AuctionFrameTab_OnClick. Vanilla 1.12 calls
	-- it as AuctionFrameTab_OnClick(index) -- the tab index is the first (often
	-- only) arg and the clicked tab button is the global `this`. AuctionTabTemplate
	-- fires it with no args at all (index nil, button = `this`). Atr_SelectPane
	-- instead calls us Wrath-style as (tabButton, index). Normalise all three
	-- into (self, index).
	local self, index;
	if (type(arg1) == "table") then
		self  = arg1;			-- Atr_SelectPane path: (tabButton, index)
		index = arg2;
	else
		self  = this;			-- Blizzard path: (index); button is `this`
		index = arg1;
	end
	...
	-- Vanilla original is AuctionFrameTab_OnClick(index); it reads the button
	-- from `this` only when index is nil, and we always pass a resolved index.
...
function Atr_CreateAuction_OnClick ()
	...
	local stackSize = gJustPosted_StackSize;
	local numStacks = gJustPosted_NumStacks;

	if (C_AuctionHouse and C_AuctionHouse.PostItem) then

		-- Native multi-post (ClassicAPI): splits `numStacks` stacks of `stackSize`
		-- out of one bag stack and posts them server-side, firing
		-- AUCTION_MULTISELL_START/UPDATE/FAILURE (recorded by our handlers below).
		-- The source must be a BAG slot, not the sell slot -- clear the slot to
		-- unlock the stack, then find one holding the full amount.
		ClearCursor();
		...
		local bid = stackStartingPrice;
		if (not bid or bid < 1) then bid = stackBuyoutPrice; end		-- PostItem requires bid >= 1
		if (not bid or bid < 1) then bid = 1; end

		C_AuctionHouse.PostItem ({ bagID = bagID, slotIndex = slotIndex },
								 duration, stackSize, numStacks, bid, stackBuyoutPrice);
	else
		-- No ClassicAPI PostItem: vanilla single post of the loaded sell slot.
...
-- ClassicAPI's C_AuctionHouse.PostItem fires these retail multi-sell events as it
-- posts each stack. They record the run against the gJustPosted_* values set in
-- Atr_CreateAuction_OnClick. All fields are guarded (buyout may be 0 for a
-- bid-only auction; a stray event outside a post has no gJustPosted_ItemName).

local gMS_stacksPrev = 0;
...
function Atr_OnAuctionMultiSellUpdate()
	...
	if (not gJustPosted_ItemName) then return; end

	local stacksSoFar	= arg1 or 0;
	local stacksTotal	= arg2;
	local stackSize		= gJustPosted_StackSize or 1;
	local buyout		= gJustPosted_BuyoutPrice or 0;

	local delta = stacksSoFar - gMS_stacksPrev;
	gMS_stacksPrev = stacksSoFar;

	if (delta > 0) then
		Atr_AddToScan (gJustPosted_ItemName, stackSize, buyout, delta);
	end

	if (stacksTotal and stacksSoFar == stacksTotal) then
		Atr_LogMsg (gJustPosted_ItemLink, stackSize, buyout, stacksTotal);
		Atr_AddHistoricalPrice (gJustPosted_ItemName, buyout / stackSize, stackSize, gJustPosted_ItemLink);
...
function Atr_OnAuctionMultiSellFailure()
	...
	-- Some stacks may have posted before the abort; log what landed.
	if (gMS_stacksPrev > 0) then
		Atr_LogMsg (gJustPosted_ItemLink, gJustPosted_StackSize or 1, gJustPosted_BuyoutPrice or 0, gMS_stacksPrev);
	end
...
function Atr_ResetDuration()
	...
	if (AUCTIONATOR_DEF_DURATION == "S") then UIDropDownMenu_SetSelectedValue(Atr_Duration, 120);  end;
	if (AUCTIONATOR_DEF_DURATION == "M") then UIDropDownMenu_SetSelectedValue(Atr_Duration, 480);  end;
	if (AUCTIONATOR_DEF_DURATION == "L") then UIDropDownMenu_SetSelectedValue(Atr_Duration, 1440); end;
...
-- 1.12 calling convention: ChatFrame_OnEvent(event); arg1..arg9 are engine globals.
function auctionator_ChatFrame_OnEvent(event)
	...
	return auctionator_orig_ChatFrame_OnEvent (event);
...
function Atr_AddMainPanel ()
	...
	UIDropDownMenu_SetWidth (150, Atr_DropDownSL);
	UIDropDownMenu_JustifyText ("CENTER", Atr_DropDownSL);
	
	UIDropDownMenu_SetWidth (95, Atr_Duration);
...
function Atr_AddSellTab (tabtext, whichTab)
	...
	local font = getglobal("AtrFontOrange");
	if frame.SetNormalFontObject then
		frame:SetNormalFontObject(font);
	elseif frame.SetTextFontObject then
		frame:SetTextFontObject(font);
	end
...
function Atr_OnSearchComplete ()
	...
	if (Atr_IsModeCreateAuction()) then
		...
		if (table.getn(gCurrentPane.activeScan.scanData) == 0) then
			gCurrentPane.hints = Atr_BuildHints (gCurrentPane.activeScan.itemName);
			if (table.getn(gCurrentPane.hints) > 0) then
...
function Atr_Condense_History (itemname)
	...
	for x = 1,table.getn(tempHistory) do
...
function Atr_Process_Historydata ()
	...
	if (table.getn(gCurrentPane.sortedHist) > 0) then
...
function Atr_UpdateRecommendation (updatePrices)
	...
	elseif (Atr_ShowingCurrentAuctions()) then
		...
		if (table.getn(gCurrentPane.activeScan.sortedData) == 0) then
...
function Atr_SetTextureButton (elementName, count, itemlink)
	...
	-- ClassicAPI's GetItemIcon strictly requires an itemID. Extract it from
	-- the link first, and tolerate nil (callers pass nil to clear the icon).
	local itemID
...
function Atr_GetUCIcon (itemName)
	...
	-- 1.12.1 has no |T...|t inline texture escape (added in 2.0).
	-- Replace with a color-coded ASCII marker:
	--   '-' gray   : no scan data
	--   '>' green  : your best is at/below the lowest going price (winning)
	--   '~' yellow : mixed (best is competitive, but you have higher-priced auctions being undercut)
	--   'X' red    : your best is above the lowest going price (being undercut)
	local icon = "|cff888888-|r "
...
function Atr_DisplayHlist ()
	...
	local numrows = table.getn(gHistoryItemList);
	...
	for line = 1,ITEM_HIST_NUM_LINES do
		...
		if (dataOffset <= numrows and gHistoryItemList[dataOffset]) then
			...
			Atr_SelectionHighlight(lineEntry, iName == gCurrentPane.activeSearch.searchText);
...
function Atr_ShowCurrentAuctions()
	...
	local numrows = table.getn(gCurrentPane.activeScan.sortedData);
...
function Atr_ShowHistory ()
	...
	local numrows = gCurrentPane.sortedHist and table.getn(gCurrentPane.sortedHist) or 0;
...
function Atr_FindBestHistoricalAuction()
	...
	if (gCurrentPane.sortedHist and table.getn(gCurrentPane.sortedHist) > 0) then
...
function Atr_HighlightEntry(entryIndex)
	...
	for line = 1,12 do
		...
		-- 1.12's Lock/UnlockHighlight doesn't refresh the texture cleanly
		-- on toggle and manually Hide()ing the highlight kills the hover
		-- effect. Use a custom overlay texture (Atr_SelectionHighlight)
		-- so we own the selection-visual state independently of the engine's
		-- hover-highlight machinery.
		Atr_SelectionHighlight(lineEntry, lineEntry:GetID() == entryIndex);
	end
	...
	if (Atr_ShowingCurrentAuctions() and entryIndex ~= nil and entryIndex > 0 and entryIndex <= table.getn(gCurrentPane.activeScan.sortedData)) then
...
function Atr_Duration_OnLoad(self)
	...
	UIDropDownMenu_SetSelectedValue (Atr_Duration, 120);
end
...
function Atr_Duration_Initialize()
	...
	-- Vanilla 1.12 StartAuction/CalculateAuctionDeposit take the run-time as
	-- MINUTES (120/480/1440 for short/medium/long). Wrath-era Auctionator
	-- stored 1/2/3 enum codes here; store the minute counts directly so the
	-- value flows straight into both Blizzard calls without translation.
	--
	-- AUCTION_DURATION_ONE/TWO/THREE don't exist in 1.12 (added in 3.x).
	-- Match vanilla Blizzard_AuctionUI's "N Hours" label style.
...
function Atr_Dropdown_AddPick (frame, text, value, func)
	...
	-- This client calls the button func as func(this.arg1, this.arg2), so the
	-- handlers would receive the raw value instead of the (info, frame) pair
	-- they expect. Wrap the call in a closure that supplies exactly that shape,
	-- independent of the client's dropdown calling convention.
	local handler = func or Atr_Dropdown_OnClick;
	info.func = function ()
		handler({ value = value }, frame);
	end
...
function Atr_FindHListIndexByName (itemName)
	...
	for x = 1, table.getn(gHistoryItemList) do
...
function Atr_CheckingActiveIdle()
	...
	if (gAtr_CheckingActive_State == ATR_CACT_READY) then
		...
		if (gAtr_CheckingActive_NextItemName == nil) then
			...
			gAtr_CheckingActive_NextItemName = (x > 0 and table.getn(gHistoryItemList) >= x+1) and gHistoryItemList[x+1] or nil;
...
function Atr_CancelUndercuts_CurrentScan(confirmed)
	...
	for x = table.getn(scan.sortedData),1,-1 do
...
function Atr_Hilight_Hentry(itemName)
	...
	for line = 1,ITEM_HIST_NUM_LINES do
		...
		if (dataOffset <= table.getn(gHistoryItemList) and gHistoryItemList[dataOffset]) then

			Atr_SelectionHighlight(lineEntry, gHistoryItemList[dataOffset] == itemName);
		end
	end
end
...
function Atr_Item_Autocomplete(self)
	...
	local numLists = table.getn(AUCTIONATOR_SHOPPING_LISTS);
	local n;
	
	for n = 1,numLists do
		local slist = AUCTIONATOR_SHOPPING_LISTS[n];

		local numItems = table.getn(slist.items);
		...
		if ( numItems > 0 ) then
			for i=1, numItems do
				...
				if ( name and text and (strfind(strupper(name), strupper(text), 1, 1) == 1) ) then
					self:SetText(name);
					-- 1.12 has no IME; the non-IME branch is always taken.
	...
	numItems = table.getn(gHistoryItemList);
...
-- 1.12 calling convention: FriendsFrame_OnEvent(event).
function Atr_FriendsFrame_OnEvent(event)
	...
	return auctionator_orig_FriendsFrame_OnEvent (event);
