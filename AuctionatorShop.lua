...
function Atr_ShoppingListsInit ()
	...
	local num = table.getn(AUCTIONATOR_SHOPPING_LISTS);
	local x;
...
function Atr_SList:AddItem (itemName)
	...
	if (self.isRecents) then
		...
		while (table.getn(self.items) > 50) do		-- max 50 items on recents list
...
function Atr_SList:RemoveItem (itemName)
	...
	local num = table.getn(self.items);
	local n;
...
function Atr_SList:DisplayX ()
	...
	local numrows = table.getn(self.items);
...
function Atr_Search_Onclick (opts)
	...
	currentPane:DoSearch (searchText, opts and opts.exact, nil, nil, opts);
...
function Atr_Shop_OnFinishScan ()
	...
	if (table.getn(currentPane.activeScan.sortedData) > 0) then
...
StaticPopupDialogs["ATR_DEL_SHOPPING_LIST"] = {
	...
	OnAccept = function(self)
		local x;
		for x = 1,table.getn(AUCTIONATOR_SHOPPING_LISTS) do
...
function Atr_AddToSListOnClick ()
	...
	if (gCurrentSList) then
		if (table.getn(gCurrentSList.items) >= 50) then
...
function Atr_ASDD_Class_Initialize (self)
	...
	-- This client calls the initialize func as initFunction(level), passing the
	-- menu level (a number) rather than the frame. Reference the frame directly,
	-- matching how Atr_ASDD_Subclass_Initialize does it.
	self = Atr_ASDD_Class;
